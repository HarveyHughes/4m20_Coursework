function draw_trajectory(walker)
...
%T = time step
    ...
    [traj,foot] = walker_to_traj(walker,1);
    ...
    hold on;
    cent=cent_mass(pos(:,2:6));
    m= plot(cent(1),cent(2), 'bo');
    pause(10);
    ...
    for i= 2:step
       pause(T);
       [traj,foot] = walker_to_traj(walker,i);
       pos=fwd_kin(traj,L,foot);   
       xs= pos(1,:);
       ys=pos(2,:);
       set(p,'XData',xs,'YData',ys)
       cent=cent_mass(pos(:,2:6));
       set(m,'XData',cent(1),'YData',cent(2));
    end
end
